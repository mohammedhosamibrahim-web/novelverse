...
router.get('/', (req, res) => {
  ...
  const total = db
    .prepare('SELECT COUNT(*) AS n FROM comments WHERE target_type = ? AND target_id = ?')
...
router.patch('/:id', requireAuth, (req, res) => {
  ...
  db.prepare('UPDATE comments SET content = ?, is_spoiler = ? WHERE id = ?').run(content, isSpoiler, comment.id);
  res.json({ ok: true });
});

/** DELETE /api/comments/:id — own comment, or moderators/admins delete any. */
router.delete('/:id', requireAuth, isStaff, (req, res) => {
  const comment = db.prepare('SELECT * FROM comments WHERE id = ?').get(req.params.id);
  if (!comment) return res.status(404).json({ error: 'Comment not found' });
  if (comment.user_id !== req.user.id && !['super_admin', 'moderator'].includes(req.user.role)) {
