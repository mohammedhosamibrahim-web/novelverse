...
router.post('/bookmarks', async (req, res, next) => {
  try {
    ...
    await db.prepare('INSERT OR IGNORE INTO bookmarks (user_id, target_type, target_id) VALUES (?, ?, ?)').run(
      req.user.id,
      targetType,
      targetId
    );
...
router.delete('/bookmarks', async (req, res, next) => {
  try {
    ...
    await db.prepare('DELETE FROM bookmarks WHERE user_id = ? AND target_type = ? AND target_id = ?').run(
      req.user.id,
      targetType,
      targetId
    );
...
router.get('/profile', async (req, res, next) => {
  try {
    const bookmarkCount = (await db.prepare('SELECT COUNT(*) AS n FROM bookmarks WHERE user_id = ?').get(req.user.id)).n;
    const historyCount = (await db.prepare('SELECT COUNT(*) AS n FROM reading_history WHERE user_id = ?').get(req.user.id)).n;
    const commentCount = (await db.prepare('SELECT COUNT(*) AS n FROM comments WHERE user_id = ?').get(req.user.id)).n;
    res.json({
      user: {
        id: req.user.id,
